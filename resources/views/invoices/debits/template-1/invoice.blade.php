...
if($currentPage == 1) {
    ...
    $i = (\App\Http\Controllers\Api\DebitController::$invoiceItemsPerPage * ($currentPage - 1)) + 1;
}
$sub_total = 0;
$tax_total = 0;
$total = 0;
...
<head>
    ...
    <title>{{ 'Invoice - ' . $debit->bill_number . '.pdf' }}</title>
...
<div class="container-fluid">
    ...
    <div class="row mt-3">
        ...
        <div class="col col-4 text-center">
            <h4 class="title is-4"><strong>DEBIT INVOICE</strong></h4>
        </div>
        <div class="col col-4 text-right">
            <h5><strong>INVOICE# {{ $debit->bill_number }}</strong></h5>
            <h5><strong>DATE: </strong>{{ $debit->bill_date->format("d-m-Y") }}</h5>
        </div>
    </div>
    <hr>
    <div class="row mt-3">
        ...
        <div class="col col-6">
            <div class="text-right">
                ...
                <address>
                    <h5><strong>{{ $debit->vendor->name }}</strong></h5>
                    {{ $debit->vendor->address_line_1 }}<br>
                    {{ $debit->vendor->city }}, {{ $debit->vendor->pin }}<br>
                    Phone : {{ $debit->vendor->contact_phone }}<br>
                    @if($debit->vendor->contact_email != null || $debit->vendor->contact_email != '')
                        Email : {{ $debit->vendor->contact_email }}<br>
                    @endif
                    <h6><strong>{{ $debit->vendor->idType->name }}# </strong>{{ $debit->vendor->identification }}</h6>
                </address>
            </div>
        </div>
    </div>
    ...
    <div class="row mt-4">
        <div class="col col-8">
            <?php
                $round_off_amount = round($debit->total);
                $round_off = $round_off_amount - $debit->total;
        ...
        <div class="col col-3 offset-1 float-right">
            <h5><strong>DEBIT SUMMARY</strong></h5>
            <table class="table table-bordered table-sm">
                <tbody>
                <tr>
                    <th class="text-center" style="">Subtotal : </th>
                    <td class="text-right" style="font-size: 15px;font-family: 'DejaVu Sans', sans-serif">
                        <strong>₹{{ number_format($debit->sub_total, 2, '.', ',') }}</strong>
                    </td>
                </tr>
                <tr>
                    <th class="text-center" style="">Tax : </th>
                    <td class="text-right" style="font-size: 15px;font-family: 'DejaVu Sans', sans-serif">
                        <strong>₹{{ number_format($debit->tax, 2, '.', ',') }}</strong>
                    </td>
                </tr>
                <tr>
