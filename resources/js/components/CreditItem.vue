<template>
    <div class="box credit-item-container mt-1">
                        ...
                        <div class="field">
                            <div class="control">
                                <div class="select is-fullwidth" :class="{'is-loading' : isStockLoading}">
                                    <select label="Select Batch" v-model="creditItem.product_stock_id" @change="onStockSelected">
                    ...
                    <div class="column is-3">
                        <div class="field is-horizontal">
                            ...
                            <div class="field-body">
                                <div class="field">
                                    <p class="control">
                                        <input class="input" placeholder="Price" type="number" v-model="creditItem.price" value="0">
                                    </p>
                                </div>
                            </div>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            getTax() {
                let taxExcludedPrice = (this.creditItem.price / (( this.creditItem.tax_percent / 100 ) + 1 ));
                let tax = Number(this.currentStock.tax_percent * taxExcludedPrice * this.creditItem.quantity / 100);
                return tax.toFixed(2);
            },
            getItemTotal() {
                let taxExcludedPrice = (this.creditItem.price / (( this.creditItem.tax_percent / 100 ) + 1 ));
                let itemTotal = Number((taxExcludedPrice * this.creditItem.quantity) + Number(this.getTax));
        ...
        methods: {
            ...
            fetchProduct() {
                axios.get('/api/v1/products/' + this.creditItem.product_stock.product.id).then(response => {
            ...
            fetchManufacturer() {
                axios.get('/api/v1/manufacturers/' + this.creditItem.product_stock.manufacturer_id).then(response => {
            ...
            onManufacturerSelected() {
                ...
                axios.get(
                    '/api/v1/products/' +
                    this.product_id +
                    '/stock/search?manufacturer_id=' + this.manufacturer_id + '&all_stock=true').then(response => {
            ...
            onStockSelected() {
                ...
                this.currentStock = this.getProductStock(this.creditItem.product_stock_id)
                this.creditItem.price = this.currentStock.price;
                this.creditItem.quantity = 1;
                this.creditItem.stockAvailable = this.currentStock.stock;
                this.creditItem.tax = this.currentStock.tax;
                this.creditItem.name = this.getProduct(this.product_id).name
            },
            getProduct(id) {
            ...
            deleteComponent() {
                this.$emit('on-delete', this.creditItem.id);
            },
            onProductCleared() {
                this.manufacturers = []
                this.stockList = []
                this.manufacturer_id = ""
                this.creditItem.product_stock_id = ""
                this.currentStock = null
                this.creditItem.price = 0
            ...
            buildCreditItem() {
                ...
                this.manufacturer_id = this.creditItem.product_stock.manufacturer_id
                this.product_id = this.creditItem.product_stock.product.id
                this.stockList.push(this.creditItem.product_stock)
                this.currentStock = this.creditItem.product_stock
                this.creditItem.price += (this.creditItem.tax / this.creditItem.quantity)
                this.isStockLoading = false;
                this.stockSelected = true;
            },
            handleError(error) {
